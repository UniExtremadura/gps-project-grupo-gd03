...
class SpotifyPlaylistTrackUnitTest {

    private fun assertFieldEquals(fieldName: String, value: Any,
        spotifyPlaylistTrack: SpotifyPlaylistTrack
    ) {
        val field = spotifyPlaylistTrack.javaClass.getDeclaredField(fieldName)
        field.isAccessible = true
        assertEquals("$fieldName fields didn't match", field.get(spotifyPlaylistTrack),
            value)
    }

    @Test
    fun checkSpotifyPlaylistTrackFields() {
        val trackValue = SpotifyTrack(
            SpotifyAlbum(
        ...
        val spotifyPlaylistTrack = SpotifyPlaylistTrack(trackValue)
        assertFieldEquals("track", trackValue, spotifyPlaylistTrack)
    }
